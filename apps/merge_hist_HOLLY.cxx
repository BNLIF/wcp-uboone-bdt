...
int main( int argc, char** argv )
{
  ...
  for (auto it = map_inputfile_info.begin(); it != map_inputfile_info.end(); it++){
    ...
    std::cout << "DEBUG: opening ROOT file -> " << out_filename << std::endl;
    temp_file = new TFile(out_filename);
    T = (TTree*)temp_file->Get("T");
    T->SetBranchAddress("pot",&pot);
    T->GetEntry(0);

    if (filetype==15 || filetype == 150){
...
std::cout << "DEBUG: map_obsch_infos size = "
          << map_obsch_infos.size() << std::endl;
  for (auto it = map_obsch_infos.begin(); it != map_obsch_infos.end(); it++){
    int obsch = it->first;
    std::cout << obsch << std::endl;
    auto bayes_inputs = it->second;
    std::cout << "obsch " << obsch << " has " << bayes_inputs.size() << " cov channels" << std::endl;
    for (size_t i=0;i<bayes_inputs.size();i++){
        int covch = std::get<3>(bayes_inputs[i].front());
        std::cout << "  covch: " << covch << " bins: " << bayes_inputs[i].size() << std::endl;
    }
}
  ...
  if (flag_err == 1){
    ...
    for (auto it = map_obsch_infos.begin(); it != map_obsch_infos.end(); it++){
      ...
      std::cout << "bayes_inputs size = " << bayes_inputs.size() << std::endl;
      for (size_t i=0;i!=bayes_inputs.size(); i++){
	int covch = std::get<3>(bayes_inputs.at(i).front());

        std::cout << "covch = " << covch << std::endl;	
